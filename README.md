...
│   ├── 项目说明.md                   ★ 完整说明书：AI 通读一遍即可理解项目（定位 / 结构 / 目录职责 /
│   │                              模块依赖 / 主要流程 / 配置运行 / 收尾清单 / 文档地图）
│   ├── 项目文档.md                   前端时代的项目文档（页面级细节；全貌以「项目说明.md」为准）
...
| 更新源 | 适用场景 | 需要令牌 |
|---|---|---|
| GitHub Releases | 默认。本仓库是**私有库**，匿名读不到 release，所以必须配令牌 | 是 |
| 自定义 URL | 以后把安装包放到公开 CDN / 自建静态站 —— 那时就不需要令牌了 | 否 |
| 本地目录 | 离线、内网，或者"我刚打完包，让装好的应用直接升级" | 否 |

令牌建议用**细粒度 PAT**：只勾这一个仓库的 `Contents: Read` 就够了。它只存在本机配置文件（`%APPDATA%\即梦批量生成控制台\desktop-config.json`）里，**不会进安装包** —— 安装包是可解压的，把令牌打进去等于公开仓库读权限。
...
- **更新源三种模式**：`github`（默认；本仓库私有，匿名读不到 release，必须配只读令牌）/ `url`（公开 CDN 或自建静态站 —— 那时不需要令牌）/ `local`（离线、内网，或开发机自测："刚打完包，让装好的应用直接升级"）。
