...
**界面层级**（UI 上叫「项目 / 页面」，代码内部是 `Project` / `Workspace`）：

```
首页（项目列表）→ 项目主页（页面列表 + 资产库 + 生成记录 + 项目设置）→ 工作区视图（分镜表）
```

- **首页** `/`：项目卡片墙，每张显示页面数 / 分镜数 / 素材数 / 最近修改；可创建、重命名、软删除。
- **项目主页** `/?project=pj_x`：四个**内容页签**，切换时页面头与导航栏保持不动，只换下方内容 ——
  **页面**（列表，新建 / 重命名 / 软删除）、**资产库**（项目级，6 类分类 / 搜索 / 上传）、
  **生成记录**、**项目设置**。后两者是**就地**嵌进来的（复用既有的记录视图与设置面板，
  不重做一套），从控制台顶栏打开时它们仍是原来的整屏 / 抽屉形态。
- **工作区视图**：就是既有那张分镜表。顶部面包屑显示「首页 › 项目 › 页面」，可逐级返回。
- **刷新恢复**：URL 带 `?project=…&workspace=…`，刷新回到原处；指向已删除对象时逐级安全降级，不会崩。
- **切换即清理**：切换项目/页面会清空勾选、筛选、搜索、弹层、预览与所有中间态，并给轮询打**代际令牌**
  丢弃过期响应 —— 避免"在 A 页面勾的分镜提交到了 B 页面"这类串线。

> 新建项目会**自动创建一个「默认页面」**，建完即可直接使用，不会出现"建了项目却什么都做不了"的死角。
> 项目下最后一个页面不允许删除（请先新建页面，或删除整个项目）。
...
测试：`node --test server/*.test.js`（共 54 项：模型上限 6 · 状态机 5 · Worker 守卫 8 · 迁移 10 · 项目隔离 25；项目无 npm 依赖，用 Node 自带测试运行器）。
