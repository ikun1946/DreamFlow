...
| 项 | 值 |
| --- | --- |
| 当前版本 | `0.37.4`（唯一生效来源：`package.json`；`README`「当前版本」与 `docs/项目文档.md` 必须同步） |
...
| 自动化测试 | **209 个用例**（`npm test`，Node 内置 test runner） |
...
门禁由 `npm test`（209 用例）、`npm run check`（45 项一致性检查）、`npm run lint`（7 项静态检查）、`npm run smoke:web`、`npm run e2e` 组成，`npm run verify` 一键串起。**这些数字由 `check-project.js` 第 16 节自动对账**，对不上就报错，所以不会悄悄过期。
