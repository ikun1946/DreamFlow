...
#### `0.37.5` — 2026-09-25（回退：内容整体回到 v0.37.1，撤销 0.37.2-0.37.4）

**为什么回退**：使用者升级到 0.37.4 后报告应用出现严重异常，且 0.37.4 是**两个并行会话的
改动合并发布**的（动画修复 + 设置页文案精简），无法快速定位是哪一半的问题，按"先回稳、
再查因"处理 —— 使用者明确要求代码全部回退到 v0.37.1。桌面日志仅有的一条线索：
`渲染进程退出：{"reason":"crashed","exitCode":-1}`（01:14，界面层崩溃；0.37.4 安装后会话
的日志没有报错），根因待查。

**回退方式**：`git revert v0.37.1..HEAD`（不重写公开历史），main 内容与 `v0.37.1` 标签
完全一致。版本号走前进到 0.37.5 —— **应用内更新器不会降级**，只有发布一个版本号更大、
内容等于 v0.37.1 的版本，已升级到 0.37.4 的用户才能通过「检查更新」回到稳定内容。

**随回退暂时消失的三个修复**（根因定位后可重新做回主线）：
- 0.37.2 设置页「检查更新」的下载状态与进度实时刷新
- 0.37.3 重启后首开设置从右侧飞入（改为居中弹出）
- 0.37.4 首开设置的合成器预热（消卡顿）与设置页文案精简
这三条的完整记录见 git 标签 `v0.37.2` / `v0.37.3` / `v0.37.4` 对应的提交与当时的
CHANGELOG 条目（`git show v0.37.4:docs/CHANGELOG.md` 可查回）。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 209/209 · `smoke:web` ✓ · `e2e` 53/53；
`git diff v0.37.1` 为空（内容一致性核对）。

**版本**：`0.37.1 → 0.37.5`（内容回退，号段越过已撤销的 0.37.2-0.37.4 防止与应用内
更新混淆）。发版后撤下 v0.37.4 的 Release（tag 保留作历史）。
