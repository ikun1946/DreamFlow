...
#### `0.38.10` — 2026-09-25（修复：切换素材槽位显示模式后不立即生效）
...
浏览器实测（隔离环境 9 张满额）：deck → scroll 切换后负 margin 立即清零、
`overflow-x:auto` 生效可横向滚动；scroll → deck 切回后牌堆立即恢复。

**版本**：`0.38.9 → 0.38.10`（PATCH：缺陷修复，无接口变更、无数据变更）。
