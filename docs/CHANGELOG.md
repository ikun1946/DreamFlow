...
#### `0.38.12` — 2026-09-25（文档：图片资产 GPT 生图实施计划）

**为什么改**：拟为每张图片资产的预览窗口加入 Work Fisher 文生图能力。素材详情有提示词编辑框，分镜绑定图预览却只有资产简要快照；而更换资产文件会同时影响所有引用它的分镜。若直接接上“生成并覆盖”，容易出现提示词取错、付费任务重复提交和共用素材被无意替换。

**改动**：新增 `docs/图片资产GPT生图实施计划.md`，写清用户从配置密钥、提交任务、查看状态到比较并采用结果的操作流程，以及服务商接口、任务持久化、文件与密钥安全、开发顺序和验收清单。项目文档的文档地图增加入口。本次只提供方案，生图按钮与接口尚未实现。

**踩过的坑**：服务商图片接口路径为单数 `/v1/image/generations`，结果是异步任务且直链有时效；提交超时又没有任务 ID 时不能自动重发可能已计费的任务。计划专门列出这些状态与处理方式，避免实施时被当作普通同步上传。

**验证**：`npm run verify`、`npm run smoke:web -- --port 8899`、`npm run e2e` 通过；默认 smoke 端口 8787 曾被其它监听者占用，隔离端口重跑后通过。

**版本**：`0.38.11 → 0.38.12`（PATCH：仅新增实施文档，无公开 API、数据格式或运行时行为变更）。
...
**为什么改**：数据目录卡片沿用通用设置行的两端对齐规则，当前路径被推到卡片中间，与新位置输入框不齐；窗口变窄后标签、路径和按钮的换行也缺乏明确顺序。

**改动**：为数据目录卡片设置独立的标签、路径和按钮列，两个切换操作放在输入区下方。当前路径单行省略，悬停可看完整路径，并向辅助技术提供完整文本；窄卡片按容器宽度把标签移到控件上方。目录选择、迁移确认和重启逻辑不变。

**踩过的坑**：右侧抽屉固定为 480px，即使桌面窗口很宽也会变窄，因此响应式布局应依据卡片宽度，不能只看视口宽度。

**验证**：`npm run verify`、`npm run smoke:web`、`npm run e2e` 通过；隔离数据环境中检查居中窗口、480px 抽屉、模拟 320px 抽屉、明暗主题、长路径、不可更改状态与待重启提示，窄布局无横向溢出。

**改动二 · 调整：分镜表提示词框的复制 / 全屏按钮统一为详情同款，去掉按钮文字**

**为什么改**：同一个"复制提示词"动作在两处长着两套皮肤 —— 分镜表提示词框右上角那两颗悬浮按钮
是自创的一套半透明底 + 顶部棱线，而「分镜 N · 详情」弹窗里的复制钮是白 veil 磨砂 + 文字标签。
使用者定标：**皮肤以详情那颗为准**，并要求**去掉按钮上的文字**（只留图标）。

**改动（`app/app.js` + `app/styles.css`）**：
- 新增 `.glass-btn` 统一皮肤：白 veil 磨砂底 + 白 veil 描边 + `blur(4px)` + 6px 圆角；
  hover → 实心主色；`.done` → 实心绿；深色底反色（`.on-dark`）同款。
  取值**逐项对齐** `.copywrap .copy-btn`（详情那颗），两处选择器不同、无法合并，改动必须同步。
- 分镜表那两颗按钮改用 `.glass-btn.icon`（24×24 纯图标）：复制 + 全屏，都不带文字。
- 定位从「两颗按钮各自写死 `right:8px` / `right:38px`」改为 flex 容器 `.prompt-acts`：
  按钮尺寸一改（这次加过文字标签、又去掉）写死偏移就要重算，容器法不用管按钮多宽。
- 隐形时补 `pointer-events:none`（悬停才开）—— 否则它会静默吃掉文本框右上角的点击。
- 复制反馈复用 `flashCopied`：图标按钮没有 `<span>`，只短暂变绿、不显示「已复制」（该函数本就兼容）。

**踩过的坑**：
- **皮肤类里不能写定位属性**。第一版把 `position:relative` 写进 `.glass-btn`，直接顶掉了调用方
  `.prompt-copy` 的 `position:absolute` —— 表现为"按钮跑到文本框外面、还像压在文字底下"。
- **自创"iPhone 液态玻璃"在这颗按钮上不成立**：它背后是 `.prompt-ta{background:transparent}` →
  纯白表格行，**没有颜色可折射**，`blur + saturate(180%)` 与顶部镜面棱线怎么调都不像；中途还试过
  用外投影补可见度，观感反而变成"奇怪的立体感"。最终按使用者指定改用详情那颗的现成皮肤，不再自创。

**验证**：`build` ✓（dist 已重建）· `lint` 7/7 · `test` 211/212 · `smoke:web` 13/14 · `check` 43/47。
后三者那些红项是**同一个环境原因**，与本次改动无关：本机跑过开发服务器，磁盘上留下了
`server/data`（含使用者的测试项目），而 test / smoke / check 各有一条「仓库内不应出现 server/data」
的卫生哨兵（check §5 还会因此把总项数从 46 抬到 47，连带 §16 的数量口径校验报错）。
浏览器实测：本机开发服务器上悬停分镜提示词框 → 两颗按钮浮现，hover 变蓝、点击复制后短暂变绿
（由使用者确认。⚠ 本机开着代理时，agent 的自动化浏览器打不开 `127.0.0.1` —— 本地回环会被代理拦住，
要复跑请给浏览器加 `localhost` 例外，或临时关代理）。

**版本**：`0.38.10 → 0.38.11`（PATCH：界面排版与按钮皮肤调整，无公开接口或数据格式变更）。

#### `0.38.10` — 2026-09-25（修复：槽位模式切换即时生效；同步发布文档）

**现象**：设置里从「堆叠牌组」切到「横向滚动」后，分镜里的图片**仍是堆叠状态**，
要等几秒（下一次轮询重绘）甚至重新进入分镜表才变样。

**根因**：模式切换只重绘了设置页 —— 表格里牌堆的**负 margin 还写在行内样式上**
（layoutDecks 的产物），切换动作没有触发重排，要等下一次轮询的整表重绘才被清掉。

**修复（`app/app.js`）**：slotmode 切换分支在 `setSlotMode` 后**立即**清空摊开状态
并调用 `layoutDecks()` 按新模式重排 —— deck → scroll 清掉负 margin（可横向滚动），
scroll → deck 恢复牌堆；同时清掉摊开状态记录（切模式后不保留摊开的浮层）。

**为什么改**：现行项目文档和发布流程仍写着「仓库没有 LICENSE」「桌面版只检测 CLI」「安装包没有签名配置」，但许可声明已入库、CLI 已支持运行时安装、签名配置与检查脚本也已具备。原有 45 项门禁仍全部通过，说明它只核对了文件与配置是否存在，没有核对现行指南如何描述它们。

**改动**：同步项目文档与发布流程的许可、CLI、FFmpeg、签名及干净 Windows 验收状态；以应用内更新为现行流程，将旧手动方案标为历史；统一变更记录的入口为本文件。`check-project.js` 增加一项只检查现行指南的事实一致性门禁，不扫描历史记录。

**踩过的坑**：签名配置存在不代表本次安装包已经签名。正式出包前必须执行 `check-signing.js --require`，出包后还要用 `--verify` 验证具体产物。文档里也不能把「旧版安装包未签名」写成对所有未来产物的永久判断。

**验证**：`npm run verify`（46 项检查、7 项 lint、212 个用例通过）、`npm run smoke:web`、`npm run e2e`（53 项断言通过）；浏览器实测（隔离环境 9 张满额）deck → scroll 切换后负 margin 立即清零，`overflow-x:auto` 生效可横向滚动，切回后牌堆立即恢复。临时注入三条旧表述，新增门禁均报错并在测试后恢复文件。

**版本**：`0.38.9 → 0.38.10`（PATCH：槽位显示缺陷修复与文档门禁修复，无公开接口或数据格式变更）。

#### `0.38.9` — 2026-09-25（调整：全屏按钮移入提示词文本框内，与复制按钮并排）

**为什么改**：使用者要求全屏按钮从标题行移入提示词文本框内，放在复制按钮右侧，
样式与出现逻辑与复制按钮完全一致（悬停浮现、半透明）。

**改动（`app/app.js` + `app/styles.css`）**：
- 标题行的全屏按钮移除；`.prompt-tawrap` 内改为两颗按钮并排 —— 复制（右 38px）、
  全屏（右 8px 贴角），共用 `.prompt-copy` 样式（悬停文本框 0.55 浮现、悬停按钮
  本体 1.0、`.15s` 过渡），点击行为不变（全屏 = 打开可编辑的全屏视图）。
- 标题行只剩时长步进器，更干净。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓。
浏览器实测（真实鼠标悬停）：两颗按钮同时浮现（opacity .55、right 38px / 8px）、
点击全屏按钮打开可编辑全屏视图、关闭自动保存行为不变；截图目检位置正确。

**版本**：`0.38.8 → 0.38.9`（PATCH：纯界面调整，无行为变更、无接口变更、无数据变更）。

#### `0.38.8` — 2026-09-25（调整：堆叠牌组按列宽实排 —— 能排几张排几张，放不下的全部叠进牌堆）

**为什么改**：使用者反馈 0.38.7 的堆叠"不要只显示两个"——应先按列宽排满，
**当图片过多会挡住其他位置时才堆叠，且有几张就叠几张**（而非固定露 2 张 + 角标）。

**改动（`app/app.js` + `app/styles.css`）**：
- 新增 `layoutDecks()`：渲染后对每个素材槽位**实测列宽**，计算最多能完整并排几张
  （F），其余 (n-F) 张全部叠进牌堆 —— 每张只露 3px 边、最上面一张完整可见；
  「＋」槽位始终保持在牌堆之后可见。叠放负 margin 逐张写入行内样式
  （margin 会吸收 flex gap，写法为 strip - 宽 - gap）。
- 触发时机：`renderTable` 末尾（绑定/解绑/轮询重绘后）、摊开与收起切换后、
  窗口缩放（120ms 防抖）——所有状态变化都经同一函数重排。
- 摊开态清空所有负 margin（浮层内素材完整显示不重叠）。
- 删除「+N」数量角标及其 CSS（牌堆本身就是数量的表达；数量仍见行内
  「参考图 N/M 张」统计）。
- 已知取舍：150px 列宽下 9 张满额时只有 1 张完整可见 + 8 张 3px 边的牌堆 ——
  列宽所限，点牌组摊开可见全部；若想要更宽的槽位列需改 `--sb-grid` 列宽定义。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓ · `e2e` 53/53。
浏览器实测（隔离环境 9 张满额）：收拢态总宽 122px ≤ 列宽 150px（此前固定 2 张 + 角标）、
牌堆 8 张叠放正确；点击摊开浮层单行、负 margin 全部清除；收起后牌堆恢复；
窗口缩放防抖重排。

**版本**：`0.38.7 → 0.38.8`（PATCH：纯显示调整，无行为变更、无接口变更、无数据变更）。

#### `0.38.7` — 2026-09-25（新增：分镜提示词悬停浮现半透明复制按钮）

**为什么改**：使用者要求"鼠标移动到分镜的提示词文本框时，右上角浮现一个半透明的
复制按钮，用于快速复制提示词"。此前复制提示词只能靠全屏视图或生成记录里的复制按钮。

**改动（`app/app.js` + `app/styles.css`）**：
- 行内提示词 textarea 外加一层 `.prompt-tawrap`（position:relative 定位锚点），
  右上角放半透明复制按钮（`I.copy` 图标，`opacity:0 → .55` 悬停浮现 → 按钮本体
  悬停 1.0，`.15s` 过渡），不占版面、不影响行高。
- 点击 = 复制文本框**当前内容**（含未保存的改动）：点击按钮先触发 textarea 失焦
  → 走既有的失焦自动保存，随后委托分支调 `copyText` 复制 —— 拿到的始终是最新文本。
  复用既有 `copyText`（clipboard API + execCommand 兜底），成功 toast「提示词已复制」。
- 事件委托挂在既有的 document click 处理器里（`data-promptcopy`），全屏编辑器
  不加此按钮（其复制需求由关闭自动保存覆盖）。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓。
浏览器实测（隔离环境 + 真实鼠标）：悬停文本框按钮浮现（opacity .55）、点击后
toast「提示词已复制」；截图目检按钮位于文本框右上角且不影响输入。

**版本**：`0.38.6 → 0.38.7`（PATCH：纯界面增强，无接口变更、无数据变更）。

#### `0.38.6` — 2026-09-25（精简：全屏编辑提示词去掉右上角保存按钮，改为关闭时自动保存）

**为什么改**：使用者反馈全屏编辑提示词右上角不需要保存按钮。按钮删除后保存语义
改为与行内文本框一致 —— **关闭全屏时自动保存**（此前"不按按钮直接关闭会丢改动"），
Ctrl+Enter 仍可随时立即保存且不关闭。

**改动（`app/app.js`）**：
- 全屏面板头删除「保存（Ctrl+Enter）」按钮，只留字数统计（悬停可见保存说明）。
- `openFullscreenShell` 增加可选 `onClose` 回调（Esc / × / 点空白关闭时触发）；
  全屏编辑器在关闭时若有未保存改动 → 自动 PATCH 落库并同步回行内文本框。
- 已知取舍：全屏里清空全部文字后直接关闭 → 弹"不能为空"提示且不落库
  （原值仍在，重开即可）。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓。
浏览器实测（隔离环境）：全屏头无保存按钮；编辑后 Esc 关闭 → 服务端 GET 持久化一致、
行内文本框同步更新。

**版本**：`0.38.5 → 0.38.6`（PATCH：交互精简，无接口变更、无数据变更）。

#### `0.38.5` — 2026-09-25（调整：堆叠牌组摊开改单行不换行 + 入场动画更柔和）

**为什么改**：使用者反馈摊开浮层里素材换行显得乱、入场动画生硬。

**改动（`app/styles.css`）**：
- 摊开浮层 `flex-wrap:wrap → nowrap`：所有素材**单行排开**（9 张 ≈ 528px 宽，
  以牌组为中心左右对称铺开）；极端超宽（如 Seedance 2.5 的 30 张）时格内
  横向滑动兜底（`overflow-x:auto` + 细滚动条），`max-width` 上限 92vw 防止出窗。
- 入场动画：`.16s ease` 直入 → **`.24s cubic-bezier(.22,1.2,.36,1)` 回弹缓动**
  （scale .88 → 1.06 轻微过冲回落），上浮感更自然。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓。
浏览器实测：9 张满额摊开后浮层内**单行**（所有元素同一 top）、宽度 528px、
动画参数生效；浮层仍以牌组为中心左右对称。

**版本**：`0.38.4 → 0.38.5`（PATCH：纯视觉调整，无行为变更、无接口变更、无数据变更）。

#### `0.38.4` — 2026-09-25（调整：堆叠牌组摊开改为原地向上浮起、向左右展开）

**为什么改**：0.39.0 的摊开浮层锚在**整表左缘**（`.cell-slots` 没有定位上下文，
absolute 落到了 `.tablecol` 上）且向下生长 —— 使用者反馈"展开不要移动到下边去"，
期望点击堆叠后**原地向上浮起、稍微放大、向左右展开**。

**改动（`app/styles.css` + `app/app.js`）**：
- `.cell-slots` 补 `position:relative` —— 摊开浮层的定位上下文从整表改为**所在单元格**；
- 浮层定位改 `left:50%;top:50%;transform:translate(-50%,-50%) scale(1.06)` ——
  以牌组为中心左右对称展开，整体上浮并放大 1.06 倍，配 `deck-pop` 入场动画
  （0.9 → 1.06 缩放淡入）与更重的悬浮阴影；
- 浮层宽度上限 340 → 460px，9 张满额时左右铺得更开。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓ · `e2e` 53/53。
浏览器实测：摊开后浮层中心与单元格中心完全重合（左右对称）、scale 1.06 生效、
9 张全可见；≤2 张的槽位点击直接预览（无可摊内容不弹层）。

**版本**：`0.38.3 → 0.38.4`（PATCH：纯视觉/定位调整，无行为变更、无接口变更、无数据变更）。

#### `0.38.3` — 2026-09-25（交互调整：提示词改为常驻文本框，全屏视图同步可编辑）

**为什么改**：0.38.0 的行内编辑走"铅笔按钮 → 编辑态"两步，使用者要求更直接：
**提示词本身就是文本框，点进去直接改**；且**全屏展开视图也要能编辑**（此前全屏是
只读着色视图）。

**改动明细（`app/app.js` + `app/styles.css`）**：
- 行内：`.prompt-text` 只读着色 span → **常驻 textarea**（maxlength 2000，无边框
  透明底融入行内，悬停虚线边 / 聚焦主色边，主题令牌适配浅 / 深色）；铅笔按钮删除。
- 保存时机：**失焦且内容有变**自动保存（PATCH prompt）+ Ctrl+Enter 立即保存；
  Esc 还原为上次保存值；空值 / 超长前端拦截。失败保留内容与脏标记可重试。
- 全屏：`data-zoom` 从只读着色视图改为**可编辑**（同一套 PATCH 保存路径，字数
  统计 + 保存按钮 + Ctrl+Enter）；打开时若行内有未保存改动会一并带上，保存后
  同步回行内并清脏标记。Esc / × / 点空白退出全屏（不保存，行内内容不动）。
- 轮询保护升级：表内存在**聚焦中或带未保存改动**的文本框时整表跳过重绘
  （此前只在"编辑态"跳过）—— 否则几秒一次的轮询会打掉焦点与输入。
- 已知取舍：素材名着色（highlightPrompt）需要只读富文本，文本框形态下不再显示；
  图号对照仍有槽位徽标与悬停提示。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓ · `e2e` 53/53。
浏览器实测（隔离环境 + 真实按键）：行内改 → 失焦自动保存 → 服务端 GET 持久化一致；
Ctrl+Enter 保存；Esc 还原不落库；全屏编辑保存后行内文本框同步更新且脏标记清除；
轮询期间输入不被打断（guard 生效）。

**版本**：`0.38.2 → 0.38.3`（PATCH：交互形式调整，无接口变更、无数据变更）。

#### `0.39.0` — 2026-09-25（新增：素材槽位两种显示模式 —— 堆叠牌组 / 横向滚动，设置里切换）

**为什么改**：0.38.0 把素材槽位改成换行后，使用者反馈"换行太乱"，并提出两种期望方案：
堆叠成扑克牌组（点开摊平）或单行横向滚动，且希望**在设置里可切换**。

**新增（前端显示偏好）**：设置 → 个性化 → **素材槽位**，两档分段控件：
- **堆叠牌组（默认 deck）**：收拢时只露前 2 张（第 2 张压上来只露一条边）+「+N」
  数量角标 +「＋」槽位，列宽内完全放得下；点牌组/角标**摊开成浮动面板**
  （盖过相邻列，z-index:6），面板内所有素材格完整可见，点击 = 原有的预览 / 替换；
  点浮层外面统一收起。摊开状态按「行:角色」记录，轮询整表重绘后不丢。
- **横向滚动（scroll）**：单行不换行，格内左右滚动，细滚动条。
- 持久化与回显完全镜像「外观」的实现：唯一事实来源 `<html data-slotmode>`，
  localStorage `jmc.slotMode` 只做启动镜像；CSS 按 `html[data-slotmode=…]` 分支；
  绝不写入 S.settings（显示偏好与生成参数无关，同主题的既有语义）。
- 槽位单元格加 `cell-slots` 类：deck 模式下 `overflow:visible`（摊开浮层要盖过
  相邻列），scroll 模式保持裁切由格内滚动条接管。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓ · `e2e` 53/53。
隔离环境浏览器实测：9/9 满额分镜在 deck 模式收拢态 138px（列宽 150px，此前换行/溢出
342px）、摊开浮层 9 张全可见；scroll 模式单行 `overflow-x:auto` 可滚；两种模式切换
即刷即生效并持久化；摊开后点素材格预览 / 替换 / × 移除 / ＋ 添加全部照常。

**版本**：`0.38.2 → 0.39.0`（MINOR：新增显示偏好能力；无接口变更、无数据变更）。

#### `0.38.2` — 2026-09-25（精简：「数据目录」卡片 —— 说明进悬停提示，重启按钮按状态出现）

**为什么改**：使用者反馈该卡片"布局太过杂乱，文字、按钮太多"——卡片常驻 4 个按钮
加一段三行的说明文字，但其中"立即重启应用"在**没做任何切换时毫无意义**，
两个动作的区别说明也只有悬停那一秒才需要看。

**改动明细（全在 `app/app.js` 的 `dataDirCardHTML`）**：
- 「打开当前目录」按钮**降级**为当前位置路径旁的小「打开」按钮（动作不丢）。
- 两个动作的完整解释**收进各自的 title 悬停提示**，删除常驻的三行说明段
  （迁移并切换：复制库到新目录、原目录保留作回退、目标必须为空；
    仅切换：只改指向不搬文件）。
- **「立即重启应用」改为状态驱动**：只有服务端状态里 `configuredDir ≠ dataDir`
  （切换过但还没重启）时才出现黄色警告条 +「立即重启」按钮 —— 待重启这个状态
  本来就该显式可见，没切换过则整块消失。此前按钮常驻，与"要不要重启"毫无关系。
- 网页版 / 环境变量锁定（canChange=false）的降级展示**原样保留**。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 212/212 · `smoke:web` ✓。
浏览器实测（桌面形态桩数据）：常规态卡片只剩 打开 / 浏览… / 迁移并切换 /
仅切换 四个控件 + 两条悬停提示，说明段清零；模拟"已切换未重启"后警告条与
立即重启按钮出现、文案完整。

**版本**：`0.38.1 → 0.38.2`（PATCH：纯界面精简，无行为变更、无接口变更、无数据变更）。
...
**修复一 · 素材槽位被图片挡住（布局缺陷）**：`.slots` 原本是不换行的 flex，素材格与
「＋」槽位都是固定宽度，列宽只有 150px —— 图片绑多了就把「＋」槽位顶出单元格
（实测 9 张图需要 472px，溢出 342px，被 `overflow:hidden` 裁掉）。使用者再也找不到
加资产的入口。**修复**：`.slots` 加 `flex-wrap:wrap`（`align-content:center` 保持
多行时垂直居中）；`.row` 的固定 `height` 改 `min-height`，让行允许长高——满额时
「＋」槽位换行到下一行，始终可见。

**修复二 · 资产数量上限统计"无法使用"**：统计（「已添加 X / 上限 Y」）其实一直存在，
但只在**点开「＋」后的资产选择弹窗里**才显示 —— 修复一里「＋」被挡住后弹窗根本
打不开，统计随之"消失"。两层修复：① 弹窗内统计保留并随勾选实时刷新（原有逻辑）；
② 分镜行的提示词元信息把「参考图 N 张」升级为**常驻的「参考图 N/M 张」**
（M = 服务端按当前模型下发的 `imageLimit`；模型未下发上限时退回只显示已用数），
不打开弹窗也能看到占用与余量。

**新增 · 分镜提示词行内编辑**：使用者反馈"分镜的提示词无法更改了"。排查结论：
`PATCH /storyboards/:id` 的契约**从一开始就支持 `prompt` 字段**（对接文档 §473 可查），
但前端从未做过编辑入口 —— `patchStoryboard` 只被用来改时长。**不是 Windows 11 的
问题，是功能缺口**。本版补上：行内铅笔按钮 → `.prompt-text` 原地换成 textarea
（1–2000 字契约前端先拦，字数实时显示；Ctrl+Enter 保存 / Esc 取消 / 按钮均可），
保存走 `Api.patchStoryboard(id, { prompt })`，成功后整行重绘（素材名着色保留）。
⚠ 编辑期间轮询触发的整表重绘会跳过（`S.editPrompt` + `data-editprompt-live`
标记守护），否则编辑器会被轮询连根拔掉；切视图后标记随 innerHTML 消失，保护自动解除。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 209/209 · `smoke:web` ✓ · `e2e` 53/53。
隔离环境浏览器实测：9/9 满额分镜的「＋」槽位换行后可见（`addVisibleInCell=true`）；
元信息显示 `参考图 9/9 张`；编辑 → 保存 → 行内重渲染 → 服务端 GET 复核持久化一致；
Esc 取消不落库。

**版本**：`0.37.5 → 0.38.0`（MINOR：新增提示词行内编辑能力；两处修复无接口变更、
无数据变更）。

#### `0.37.5` — 2026-09-25（精简：「生成引擎与账号」卡片只显示账号 / 积分 / 版本）

**为什么改**：使用者两度反馈该卡片"文字太多太杂乱"，并明确给出目标：**常驻只显示
账号、积分、版本**。此前（0.37.4 改动一）已精简过一轮说明文案，但状态区仍有四类
常驻噪音：头部说明行、「默认模型 → 创作 CLI」横幅、就绪行里的"已就绪 / 读取于 /
本机构建 / 官方当前版本"双层结构、"负责全部视频生成链路…"段落。

**改动明细（全在 `app/app.js`）**：
- **就绪态卡片**（`cliStateCardHTML` 态 3）：改为拼装式渲染，常驻**只出三项**
  `账号 X · 积分 N · 版本 V`（各项存在才显示，` · ` 连接）。「已就绪」二字删除 ——
  绿色卡片 + 对勾本身就在表达"就绪"；积分**读取时间**、**本机构建 commit**、
  **安装路径**全部降为悬停提示（排查时仍可查到，不占版面）；积分数据过期时
  在数字后补一个「（更新中…）」小字，避免误读旧值。
- **卡片头说明行**删除（"全部生成都由创作 CLI 执行…"与子块/标题语义重复）。
- **「默认模型 → 创作 CLI · 全部可用模型均由创作 CLI 执行」绿色横幅**删除。
  有话必须说时才出现：模型不可用（原警示卡**原样保留**）；旧名映射 / 建议改选
  （`defaultsNotice` 消息自含，改为独立提示卡，不再寄宿在被删除的横幅里）。
- **「负责全部视频生成链路…」段落**删除。「切换账号会先退出当前账号并要求确认」
  的警示**未丢失** —— 它本来就在切换按钮的 title 悬停提示里。

**坑（版本的两个来源，不能混排）**：界面上只显示**官方当前版本**
（官方 `version.json` 的语义化版本，应用内更新比对的就是它）；本机构建 commit 是
exe 自己报的另一个来源，只进 tooltip 且标注「本机构建」二字 —— 两者的判据见
AGENTS.md「创作 CLI 的安装能力」一节。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 209/209 · `smoke:web` ✓。
浏览器实测（隔离数据目录 + 真实 CLI 探测）：就绪行渲染恰为
`账号 171979828363885 · 积分 0 · 版本 1.4.18`；"已就绪 / 负责全部视频生成链路 /
全部可用模型均由 / 读取于"在卡片渲染文本中全部清零；tooltip 含安装位置与本机构建。
（注：本条目曾随 0.37.5 回退一波三折 —— 改动先 stash 后恢复，见 2721319/479c880
两条历史提交；最终按使用者决定保留精简、不回退。）

**版本**：`0.37.4 → 0.37.5`（PATCH：纯界面文字精简，无行为变更、无接口变更、无数据变更）。

#### `0.37.4` — 2026-09-25（两项：设置页文案精简 · 首开设置卡顿修复）

> 本版本合并了两个同日完成的独立改动（各自验证通过后共用一个版本号提交）。

**改动一 · 精简：设置页「个性化」「生成引擎与账号」的说明文案**

**为什么改**：使用者反馈这两个模块"文字说明太多"。核对属实 —— 说明里混了大量给开发者
看的内容：实现细节（`#app` 上的 `.compact` 类、`localStorage` 键名 `jmc.theme`、行高
132px 这类像素值）、CLI 内部机制（OAuth 登录态、`relogin` 语义）、历史叙事（"与原来
顶栏那个按钮是同一套逻辑，入口挪到了这里"——这是变更记录的事，不是界面的事），把真正
要紧的动作指引淹没了。

**原则**：说明只留一句「是什么」；操作细节移进 tooltip（悬停可见，不占版面）；三类信息
**不可删** —— 下一步动作指引、信任关键点（即梦官方源、装到哪、官方当前版本）、错误态里
的排查指引。**代码注释一律未动**（被删的文案细节在 CHANGELOG 历史条目里都可查回）。

**改动明细（全在 `app/app.js`，纯字符串改动）**：
- 个性化：卡片说明压成「只影响界面显示，不改动生成参数」；紧凑视图 ~110 字说明段整段
  删除，要点并入开关 tooltip；外观 ~90 字说明压成一行（只解释"跟随系统"的行为），
  保存位置进分段控件 tooltip。
- 生成引擎与账号：卡片说明两句并一句（原第二句与下方状态卡重复）；CLI 子块 ~110 字
  说明压成一句（"切换账号"会先退出账号并要求确认 —— 这个用户必须预先知道，保留）；
  态 1 未安装压成一句动作指引（"从即梦官方源下载"这个信任点保留）；态 3 已就绪的
  长安装路径挪进卡片 tooltip，版面只留账号 / 积分 / 构建 / 官方版本。
- 态 2（未登录）/ 态 2b（不可用）/ CLI 更新提示 / 待授权 四段错误态文案**原样保留**：
  经逐句核对，每句都是动作指引或排查信息，删任何一句都伤使用者。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 209/209 · `e2e` 53/53 ·
`smoke:web` 通过（首跑 2 项失败与本改动无关：上一轮 0.37.3 动画验证的后台服务被
TaskStop 杀壳时留下孤儿 node 进程占着 8787，导致 smoke 的横幅断言与端口释放断言失败，
清掉孤儿后通过 —— TaskStop 杀壳不杀子进程树，Windows 上起后台服务要记得收尸）。
浏览器实测：旧文案五个关键词（132px / `.compact` / `jmc.theme` / `relogin` / `OAuth`）
在设置页全部清零；三处 tooltip（紧凑视图开关、外观分段、就绪卡）内容正确；截图目检
两张卡片版面正常。

**改动二 · 修复：第一次启动应用点开设置会卡一下**

**现象**：冷启动后**第一次**点开「设置」（居中弹窗态），入场动画中途整个窗口冻结
约 0.1 秒；二次打开起完全顺滑。

**根因（在合成器/GPU 侧，不在 JS）**：首开的居中弹窗是"新图层 + 820px 大阴影面板 +
全屏遮罩"的组合，它们的**首次**图层创建与栅格化会让合成器停顿 ~60-80ms。诊断时排除了
三个假嫌疑人：渲染主线程全程空闲（5ms 定时器流零空洞，无 50ms+ longtask）—— 说明不是
JS/布局；接口数据到达后的整面板 innerHTML 重建虽与动画窗口重叠，但推迟到动画结束后
掉帧依旧 —— 也不是它；will-change 预提升图层、单独去掉阴影均无效 —— 说明不是某个
单一纹理，而是合成路径的一次性初始化成本（着色器编译 / 命令缓冲 / 首次纹理上传）。
rAF 帧流在此期间出现空洞但定时器照常走，这正是"合成器停顿"与"主线程卡死"的鉴别特征。

**修复（`app/app.js`）**：
- **合成器预热**（主修复）：`boot()` 末尾挂 `requestIdleCallback`，把真实面板切到
  居中关闭位、真实遮罩临时显形，都以 opacity 0.01 真实渲染一帧（0 会被合成器整层
  剔除不栅格化，0.01 不可感知），把一次性成本消化在用户点击之前；预热期间临时关掉
  过渡防止面板在屏幕上横扫。实测预热后首开与二开同样顺滑（连续三次冷启动零掉帧）。
- **数据重绘让位动画**（加固）：`openSettings()` 里接口返回后的 `renderSettings()`
  推迟到入场动画结束（`transitionend` + 350ms 兜底定时器）再执行，避免动画中段的
  全量重排与合成动画互相干扰。

**验证**：冷启动桌面端 CDP 实测三次，首开/二开 rAF 帧流与定时器流均零空洞（修复前
首开必有 63-83ms 合成器停顿）；预热后停靠态干净还原（无 .center / 行内样式残留），
首开关键帧仍为居中弹出 `scale(.97)→scale(1)` + 淡入（0.37.3 修复未回归）；
`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 209/209 · `e2e` 53/53 · `smoke:web` 通过。

**版本**：`0.37.3 → 0.37.4`（PATCH：两项改动均无接口变更、无数据变更）。

#### `0.37.3` — 2026-09-25（修复：重启后首开设置从右侧飞入、二次打开才正常居中弹出）

**现象**：重启应用后**第一次**点开「设置」（首页/项目墙的居中弹窗态），窗口会从屏幕右侧
一路横扫到中央；关掉再开，才是正常的"居中弹出 + 淡入"。只有首开异常，之后一直正常。

**根因（`app/app.js` 的 `openSettings()`，0.35.1 引入居中态时埋下）**：设置窗口是同一个
DOM 节点（`#settingsDrawer`）承载抽屉态与居中态两种形态，切换形态时已有"先摘布局再改类"
的保险：先 `hidden=true` 再改类，本意是让类变更落在 `display:none` 上、不产生跨屏过渡。
但首开时 `hidden=true` 与后面的 `hidden=false` 落在**同一个同步任务**里，中间没有任何
样式重算 —— `display:none` 从未成为过该元素的真实计算样式。于是"强制重排再挂 .open"
那次重排读到的"变更前样式"仍是启动时的抽屉位 `translateX(100%)`，浏览器把它当成过渡
起点，面板就从不透明地从右侧横扫到中央。关掉再开之所以一直正常，是因为 `closeSettings()`
对居中态会真的落 `hidden`，窗口在 `display:none` 里"真实停靠"过一次，再打开时没有可
过渡的旧位置 —— 这正好解释了"只有重启后第一次打开出问题"。

**修复（1 处）**：摘布局（`hidden=true`）之后**立刻强制一次重排**（`void dr.offsetWidth`），
让 `display:none` 真正成为一次"已计算样式"，后续改类 / 显形就没有可过渡的旧位置了。
一处改动覆盖所有形态切换路径（重启直开首页设置 / 抽屉态→居中态 / 就地态→居中态）；
抽屉态自身的滑入滑出不受影响（抽屉→抽屉不走这段分支，`closeSettings` 对抽屉态本就不落
`hidden`）。同时修正了原注释里"摘成 display:none 后类变更不产生过渡"的错误前提
（它只在 display:none 真实生效过之后成立）。

**验证**：`build` ✓ · `check` 45/45 · `lint` 7/7 · `test` 209/209 · `smoke:web` ✓ · `e2e` 53/53。
浏览器实测（隔离数据目录起真服务，页面内注入 rAF 采样器 + 读取过渡关键帧起止值）：
首开过渡起点从修复前的 `translateX(100%)`（右侧横扫）变为 `translate(-50%,-50%) scale(.97)`
（居中淡入）✓ · 关闭重开的关键帧与首开完全一致（首开/再开行为统一）✓ ·
center→drawer 切换后抽屉态滑入 `translateX(100%)→none` 原样保留 ✓。
（坑：页面在后台时 `document.visibilityState === 'hidden'`，CSS 过渡时间线冻结、
`getAnimations()` 的 `currentTime` 恒为 0，逐帧采样会误判"动画卡死"；此时应读
`effect.getKeyframes()` 的起止值做断言 —— 它不依赖时间线推进。）

**版本**：`0.37.2 → 0.37.3`（PATCH：缺陷修复；无接口变更、无数据变更）。
