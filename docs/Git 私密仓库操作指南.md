...
> 状态：历史（2026-09-21 起仓库已公开 —— 第 2–4 节的"建私有仓库 / 私密性验证"不再适用；第 1 节的认证与推送方式仍可参考）
> 权威范围：GitHub 认证方式、令牌权限（私密性验证部分已过时）
> 最新流程见：`docs/项目全面审查与改进流程.md`

> ⚠ **2026-09-21：本仓库（`ikun1946/DreamFlow`）已由私有改为公开。** 本文写作时它是私有库，因此第 2 节"创建私有仓库"、第 3.3 节"必须关闭的泄露面"、第 4 节"验证私密性生效"对**本仓库**不再适用（仍适用于你新建的其它私有库）。推送/认证方式（第 1、3.2、5 节）不受影响。
...
| `gh` CLI | 未安装 |
| git 身份 | `nick9715043082 <ikun1946@users.noreply.github.com>`（2026-09-21 起改用 GitHub noreply 邮箱，仓库内已无个人邮箱） |
