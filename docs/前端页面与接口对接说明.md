...
#### 4.6.5 尺寸参数（0.41.0 新增：分辨率 + 宽高比）

提交体在 `{ prompt }` 基础上**按模式二选一**（不传任何尺寸字段 = 老客户端行为，服务端归一 `size:'auto'`）：

| 模式 | 提交体字段 | 发给服务商的 `size` | 同时发 `resolution`？ |
|---|---|---|---|
| 比例 | `{ sizeMode:'ratio', ratio:'16:9', resolution:'1k' }` | 比例枚举（`'16:9'`）或 `'auto'` | **是** |
| 像素 | `{ sizeMode:'pixels', width:1920, height:1088, resolution:'2k' }` | 精确 `'1920x1088'` | **否**（像素场景服务商会忽略它） |

**规则唯一事实来源**：`server/image-size.js`。前端不做最终裁决 —— 规格经 `GET /meta/options` 的 `meta.imageSizes` 下发（比例枚举、预设、LIMITS），前端只做**即时反馈**；服务端在**计费提交之前**权威校验。

- **硬约束**（Work Fisher v2.5-flare）：宽高为 16 的倍数、均 ≤3840、长短边比 ≤3:1、总像素 655360–8294400。⚠ **1080 不是 16 的倍数**，1080p 预设 = 1920×1088。
- **联动**：选比例 → 前端按分辨率档位换算像素并填入；改像素 → 反算最接近的比例枚举并锁定。
- **非法输入回落**：前端 blur/change 即校验并**回落最近合法值**（`nearest` 保住方向与比例意图：5000×500 → 2736×912 横向 3:1，不会压成任意合法值）；服务端驳回时错误响应带 `data.nearest`，前端一键采纳并 toast 说明。
- **持久化**：设置页新增「图片生成默认尺寸」卡（全局 `settings.imageDefaults` = `{ sizeMode, ratio, width, height, resolution }`）；生图弹窗打开时预填全局默认、可临时修改（临时改动只影响本次提交，点「保存默认」才落库）。`PUT /settings` 收到非法值时**回落保存**并在 `adjustments[]` 说明；`POST /settings/reset` 支持 `imageDefaults` 作用域。
...
| 26 | POST | `/assets/{id}/image-jobs` | **提交生图（2026-09-25 新增；0.41.0 补尺寸）**：体 `{ prompt }` + 尺寸二选一（`{sizeMode:'ratio', ratio, resolution}` 或 `{sizeMode:'pixels', width, height}`，详见 §4.6.5）+ `Idempotency-Key`。进行中任务会被复用（不重复扣费）；尺寸非法 → `40001` + `data.nearest` 且**不发服务商请求** | 生图面板 | 否 |
