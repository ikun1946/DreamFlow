...
3. **更新版本号（两处三地）**：`package.json`（唯一生效来源）+ `README.md` 的「当前版本」+「变更记录」。新增能力 → MINOR，修缺陷 → PATCH。
...
| 命令 | 管什么 | 现在的状态 |
|---|---|---|
| `npm test` | 单元 / 集成：`test/*.test.js`（数据安全 / 任务逻辑 / 构建发布三组） | **85 用例全通** |
| `npm run check` | 一致性门禁：`scripts/check-project.js`（版本漂移、dist 同步、图标、许可、更新器、旧名残留…） | **35 项全通** |
...
| `npm run verify` | 一键：`check` + `test` + `build:web` | — |
...
`https://github.com/ikun1946/DreamFlow.git` 是**私有库**（未登录访问 API 返回 404）。这决定了两件事：
