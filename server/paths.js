...
/* 根目录放在一个**可替换的对象**里，而不是三个模块级常量。
   为什么：本模块会真的读写磁盘，而测试必须能把根指到临时目录 ——
   常量在 require 时就定死了，测试改 `PATHS.PROJECTS_DIR` 是改不动的
   （2026-09-20 实测踩到：测试以为沙箱化了，其实一路写进了真实的 data/projects/，
    还差点把真实文件删掉）。所有函数都在**调用时**读 roots，测试用 setRoots 即可重定向。
   ⚠ 生产代码不要调用 setRoots。 */
const roots = {
